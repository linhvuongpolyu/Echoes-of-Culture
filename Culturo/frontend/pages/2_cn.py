...
st.markdown("""
    <style>
        ...
        html, body, .stApp { background: linear-gradient(90deg, #F3E5E5 0%, #FFFFFF 100%) !important; min-height:100vh !important; }
        .main, .block-container, .stApp > header, [data-testid="stHeader"] { background:transparent !important; }
        .cn-header { display:flex; align-items:center; gap:6px; }
        ...
        button[kind="primary"] {
            ...
            background:linear-gradient(90deg, rgba(243,229,229,1) 0%, rgba(254,241,246,1) 100%) !important;
...
def china_page():
    ...
    with st.sidebar:
        ...
        if st.button("← General", key="sidebar_general_cn", width="stretch"):
        ...
        with st.expander("Discover", expanded=True):
            if st.button("China", key="nav_china", width="stretch", disabled=True):
                pass
            if st.button("Hong Kong", key="nav_hk_from_cn", width="stretch"):
    ...
    with col_left:
        ...
        with c1:
            ...
            """, unsafe_allow_html=True)
            if st.button("Click to start 🚀", key="cn_animals_btn", width="stretch", type="primary"):
                st.session_state.current_region = "China"; st.switch_page("pages/5_draw.py")
        with c2:
            ...
            """, unsafe_allow_html=True)
            if st.button("Click to start 🚀", key="cn_language_btn", width="stretch", type="primary"):
                st.session_state.current_region = "China"; st.switch_page("pages/4_lg.py")
        c3, c4 = st.columns(2)
        with c3:
            ...
            """, unsafe_allow_html=True)
            if st.button("Click to start 🚀", key="cn_arts_btn", width="stretch", type="primary"):
                st.session_state.current_region = "China"; st.switch_page("pages/7_per.py")
        with c4:
            ...
            """, unsafe_allow_html=True)
            if st.button("Click to start 🚀", key="cn_food_btn", width="stretch", type="primary"):
                st.session_state.current_region = "China"; st.switch_page("pages/6_food.py")
    with col_right:
        target_height = 520
        map_path = Path(__file__).parent.parent.parent / "assets" / "pages" / "China_main.png"
        chosen = None
        if os.path.exists(map_path):
            chosen = map_path
        else:
            for name in ["China.png", "china_map.png"]:
        ...
        if chosen:
            ...
            st.markdown("""
                <div style=\"background:#f0f0f0; border-radius:15px; padding:3rem; text-align:center; min-height:400px; display:flex; align-items:center; justify-content:center;\">\n                    <p style=\"color:#999;\">China Map</p>\n                </div>
            """, unsafe_allow_html=True)
    # Reinforce gradient override at end to ensure mint not applied from other pages
    st.markdown("""
        <style>
            html, body, .stApp { background: linear-gradient(90deg, #F3E5E5 0%, #FFFFFF 100%) !important; }
        </style>
    """, unsafe_allow_html=True)
